/*
  # Safe Fix for 750K Batch Generation Functions

  1. Approach
    - Use CASCADE to drop functions and their dependencies
    - Check for function existence before dropping
    - Recreate with all fixes applied
    
  2. Issues Fixed
    - Function ambiguity errors
    - org_record assignment error
    - timestamp type mismatch
    - RLS policy violations
*/

-- Drop all overloaded versions of the functions using CASCADE with specific argument lists
DO $$ 
BEGIN
    -- Drop generate_transaction_batch with specific argument lists
    DROP FUNCTION IF EXISTS generate_transaction_batch(INTEGER, INTEGER) CASCADE;
    DROP FUNCTION IF EXISTS generate_transaction_batch(INTEGER) CASCADE;
    DROP FUNCTION IF EXISTS generate_transaction_batch() CASCADE;
    
    -- Drop generate_full_dataset with specific argument lists
    DROP FUNCTION IF EXISTS generate_full_dataset(INTEGER, INTEGER) CASCADE;
    DROP FUNCTION IF EXISTS generate_full_dataset(INTEGER) CASCADE;
    DROP FUNCTION IF EXISTS generate_full_dataset() CASCADE;
    
    -- Drop monitor_generation_progress
...
-- Create generate_transaction_batch function
...
BEGIN
    WHILE counter < batch_size LOOP
        ...
        -- Select full organization record (FIX: was only selecting ID)
...
-- Create generate_full_dataset function
...
-- Create monitor_generation_progress function
...
BEGIN
    ...
    SELECT 
        ...
        MAX(datetime)::TIMESTAMP as latest_transaction,
        MIN(datetime)::TIMESTAMP as earliest_transaction,
...
-- Grant permissions
GRANT EXECUTE ON FUNCTION generate_transaction_batch(INTEGER, INTEGER) TO authenticated;
GRANT EXECUTE ON FUNCTION generate_full_dataset(INTEGER, INTEGER) TO authenticated;
GRANT EXECUTE ON FUNCTION monitor_generation_progress() TO authenticated;
GRANT EXECUTE ON FUNCTION generate_transaction_batch(INTEGER, INTEGER) TO anon;
GRANT EXECUTE ON FUNCTION generate_full_dataset(INTEGER, INTEGER) TO anon;
GRANT EXECUTE ON FUNCTION monitor_generation_progress() TO anon;

-- Add helpful comments
COMMENT ON FUNCTION generate_transaction_batch(INTEGER, INTEGER) IS 'Generates a batch of realistic Philippine retail transactions with SECURITY DEFINER to bypass RLS.';
COMMENT ON FUNCTION generate_full_dataset(INTEGER, INTEGER) IS 'Orchestrates generation of 750K transactions in batches with progress monitoring.';
COMMENT ON FUNCTION monitor_generation_progress() IS 'Monitors transaction generation progress with proper timestamp handling.';