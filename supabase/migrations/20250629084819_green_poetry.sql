/*
  ...
  1. Changes
    - Fix `generate_full_dataset` function to properly initialize org_record
    - Fix `monitor_generation_progress` function to handle timestamp type mismatch
    - Improve error handling in both functions

  2. Issues Fixed
    - "record org_record is not assigned yet" error in generate_full_dataset
...
-- Fix the monitor_generation_progress function to handle timestamp type mismatch
