...
-- Drop all overloaded versions of the functions using CASCADE
DO $$ 
BEGIN
    -- Drop all versions of generate_transaction_batch
    IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'generate_transaction_batch') THEN
        DROP FUNCTION IF EXISTS generate_transaction_batch CASCADE;
    END IF;
    
    -- Drop all versions of generate_full_dataset
    IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'generate_full_dataset') THEN
        DROP FUNCTION IF EXISTS generate_full_dataset CASCADE;
    END IF;
    
    -- Drop monitor_generation_progress
    IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'monitor_generation_progress') THEN
...
COMMENT ON FUNCTION generate_transaction_batch IS 'Generates a batch of realistic Philippine retail transactions with SECURITY DEFINER to bypass RLS.';
COMMENT ON FUNCTION generate_full_dataset IS 'Orchestrates generation of 750K transactions in batches with progress monitoring.';
